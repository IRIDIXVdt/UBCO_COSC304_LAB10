		...
		try ( Connection con = DriverManager.getConnection(url, uid, pw);) {	// Make the connection
			...
			while (rst.next()){	// Print out the ResultSet and For each product create a link of the form		
				...
				if(rst.getString(4) != null){
					out.print("<tr><td><div><img src="+rst.getString(4)+" class =\"i\"></div><a href=addcart.jsp?id="+rst.getInt(1)+"&name="+nameReplace+"&price="+rst.getString(3)+" class =\"b\">Add to Cart</a></td><td>"
						+ "<a href=product.jsp?id=" + rst.getInt(1)+" class =\"c\">"+rst.getString(2) + "</a></td><td>");
				}else{
					out.print("<tr><td><a href=addcart.jsp?id="+rst.getInt(1)+"&name="+nameReplace+"&price="+rst.getString(3)+" class =\"b\">Add to Cart</a></td><td>"
						+ "<a href=product.jsp?id=" + rst.getInt(1)+" class =\"c\">"+rst.getString(2) + "</a></td><td>");
				}
				if(rst2.next())		
